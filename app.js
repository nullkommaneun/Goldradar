...
function tooltipFor(key, assess, freqCfg){
  ...
  const rule =
    key === 'DFII10' || key === 'DTWEXBGS' || key === 'DCOILBRENTEU' || key === 'T10Y2Y' || key === 'NAPM'
      ...
      : key === 'VIXCLS' || key === 'T10YIE' || key === 'BAMLH0A0HYM2' || key === 'RECPROUSM156N'
...
// ---------- Portfolio: Wertberechnung ----------
let SPOT_USD_PER_KG = null;            // Primärquelle: stooq XAUUSD
let FIX_USD_PER_OZ = null;             // Fallback: LBMA Fix (USD/oz)
...
function updateHoldingsValue(){
  ...
  if (!Number.isFinite(grams) || grams<=0 || !Number.isFinite(usdPerKg)) {
    out.className = 'badge neutral';
    out.textContent = '—';
    if (note && source) note.textContent = `Preisquelle: ${source === '—' ? '—' : source}.`;
    return;
  }
  const usd = usdPerKg * (grams/1000);
  out.className = 'badge green';
  out.textContent = fmtUSD(usd);
...
(async function(){
  try{
    ...
    // Historischer Vergleich (wie gehabt)
