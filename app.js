...
const fmtUSD = (n) => new Intl.NumberFormat('de-DE',{style:'currency',currency:'USD',maximumFractionDigits:0}).format(n);
...
function renderSystemFromPreflight() {
  ...
  const h = pf.stats?.history || {}; const s = pf.stats?.spot || {};
  let parts = [];
  ...
  if (s.hasValue) {
    ...
    } else parts.push(`Spot: vorhanden`);
...
function openTooltip(btn, text){
  ...
  // Positionieren (oben bevorzugt)
  const r = btn.getBoundingClientRect();
  const margin = 8;
  const vw = Math.max(document.documentElement.clientWidth, window.innerWidth || 0);
  const vh = Math.max(document.documentElement.clientHeight, window.innerHeight || 0);

  // Temporär sichtbar messen
  el.style.left = '0px'; el.style.top = '0px';
  const ew = el.offsetWidth, eh = el.offsetHeight;

  // Standard: oben linksbündig zum Button
...
// ---------- Portfolio ----------
let SPOT_USD_PER_KG = null;  // Spot (Primär)
let FIX_USD_PER_OZ  = null;  // LBMA Fix (Fallback)
...
// ---------- App (wie zuvor, inkl. staged render & idle analogies) ----------
/* Der Rest der App bleibt identisch zu deiner letzten, performanten Version:
   - Daten laden (fetchJSON)
   - Spot/History setzen
   - Skeletons ausblenden
   - Treiber rendern (und hier neu: Info-Button als <button> mit data-tip)
   - Forecasts rendern
   - Historischer Vergleich im requestIdleCallback
...
(async function(){
  try{
    ...
    // Treiber-Element mit NEUEM Info-Button
    ...
    ["DFII10","T10YIE","T10Y2Y"].forEach(k => $rates.appendChild(driverItem(k)));
    ["DTWEXBGS","VIXCLS","BAMLH0A0HYM2","RECPROUSM156N"].forEach(k => $risk.appendChild(driverItem(k)));
    ["DCOILBRENTEU","NAPM"].forEach(k => $real.appendChild(driverItem(k)));

    const sum = summarize(assess);
    document.getElementById('drivers-sum').textContent = `In Summe: ${sum.text}`;
    document.getElementById('gs-rates').className = `dot ${computeGroupStatus(["DFII10","T10YIE","T10Y2Y"], assess)}`;
    document.getElementById('gs-risk').className  = `dot ${computeGroupStatus(["DTWEXBGS","VIXCLS","BAMLH0A0HYM2","RECPROUSM156N"], assess)}`;
    document.getElementById('gs-real').className  = `dot ${computeGroupStatus(["DCOILBRENTEU","NAPM"], assess)}`;
    ...
    // Historischer Vergleich im Idle
