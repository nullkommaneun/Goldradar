...
// Frequenzen & Schrittweite (in **Beobachtungen**, nicht Kalendertagen)
const FREQ = {
...
function buildSeriesMap(rows){
  // Für jeden Treiber: Liste gültiger Beobachtungen [{date, value}], sortiert.
  const map = {};
  for(const k of DRIVER_KEYS){
    const arr = rows
...
function currentDeltaFromSeries(seriesArr, steps){
  // letzter vs. n-ter vorheriger gültiger Wert (reine Serienlogik)
...
function currentDeltaMonthly(seriesArr){
  // MoM: letzter vs. vorheriger gültiger
...
function dateToSeriesIndex(seriesArr){
  // Liefert eine Funktion f(dateStr) -> größter Index j mit date_j <= dateStr, sonst -1.
...
function historicalDeltaAtDate(seriesArr, idxFinder, steps, dateStr, isMonthly){
  // Delta zur gegebenen globalen Zeit (über Serienindizes)
...
(async function(){
  try{
    ...
    Object.keys(labels).forEach(k=>{
      ...
      const cfg = FREQ[k] || {label:""};
      const dval = isFinite(latestDelta[k]) ? ((latestDelta[k]>=0?'+':'')+(100*latestDelta[k]).toFixed(1)+'%'+cfg.label) : '—';
    ...
    // Für jedes globale Datum: Delta je Serie per **Serienindex** bestimmen
    ...
    // Anzeige: Top ≥60% ODER bester Treffer
